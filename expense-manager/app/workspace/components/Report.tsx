...
/* ========= API TYPES (what backend sends) ========= */
type ChartAPI = {
  monthId: string
  fields: Record<string, number> // ← backend sends object
}

/* ========= UI TYPES (what UI uses) ========= */
type Field = {
  title: string
  value: number
}
...
const Report = () => {
  ...
  const getReports = async () => {
    ...
    try {
      const res = await axios.get<ChartAPI[]>(
        `${rootURL}/accounts/${accountId}/reports`
      )
      ...
      const transformed: Chart[] = res.data.map((item) => ({
        ...
        fields: Object.entries(item.fields ?? {}).map(
          ([key, value]) => ({
            title: key,
            value: value,
          })
  ...
  <div className="space-y-5">
    {reports.map((chart) => {
      const total = chart.fields.reduce(
        (sum, f) => sum + f.value,
        0
      )
      ...
      return (
        <div
          key={chart.monthId}
          className="border border-gray-200 rounded-lg bg-gray-50 p-5"
        >
          {/* Month ID */}
