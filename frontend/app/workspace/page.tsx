  ...
  useEffect(() => {
    ...
    const fetchMonths = async () => {
      try {
        ...
        setMonthList(data);
        if (data.length > 0) setTargetedMonth(data[0]);
      } catch (err) {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex w-64 bg-white border-r">
          <div className="w-16 bg-teal-800 flex flex-col items-center py-4">
            <SidebarBtn
              active={selectedPanel === Panels.WORKSPACE}
              onClick={() => setSelectedPanel(Panels.WORKSPACE)}
            >
              <UserPen />
            </SidebarBtn>
            <SidebarBtn
              active={selectedPanel === Panels.EVENT}
              onClick={() => setSelectedPanel(Panels.EVENT)}
            >
              <CalendarDays />
            </SidebarBtn>
            <SidebarBtn
              active={selectedPanel === Panels.REPORT}
              ...
              <div className="space-y-3">
                {monthList.map((m, i) => (
                  <div key={i} className="cursor-pointer hover:underline font-medium">
                    <button
                      onClick={() => setTargetedMonth(m)}
                      className={`text-teal-800 cursor-pointer ${targetedMonth === m ? 'underline' : ''}`}
                    >
                      {m}
                    </button>
                  </div>
        ...
        <div className="flex-1 p-6 overflow-hidden">
          ...
          {selectedPanel === Panels.EVENT && (
            <div className="h-full flex items-center justify-center text-2xl font-bold">Events</div>
          )}
...
const SidebarBtn = ({ active, onClick, children }: SidebarBtnProps) => (
  <button
    onClick={onClick}
